...
#define GLCHECK(command)                                                       \
  ...
  if (int err = glGetError())                                                  \
    LOG(ERROR) << "GL error detected: " << err;
#else
...
private:
  ...
  std::vector<GLfloat> anchors_depth_;
  GLuint depth_texture_n_ = 0;
  ...
  GLint depth_texture_uniform_ = -1;
  GLint anchors_depth_uniform_ = -1;
  ...
  absl::Status GlBind(const TriangleMesh &triangle_mesh,
                      const GlTexture &texture, const float *depth_texture);
  absl::Status GlRender(const TriangleMesh &triangle_mesh,
                        const float *model_matrix);
  absl::Status GlRender(const TriangleMesh &triangle_mesh,
                        const float *model_matrix, const float &anchor_depth);
...
absl::Status GlAnimationOverlayCalculator::GetContract(CalculatorContract *cc) {
  ...
  // Depth texture
  if (cc->Inputs().HasTag("DEPTH_TEXTURE")) {
    cc->Inputs().Tag("DEPTH_TEXTURE").Set<AssetTextureFormat>();
  }

  if (cc->Inputs().HasTag("ANCHORS")) {
...
void GlAnimationOverlayCalculator::LoadModelMatrices(
  ...
  for (int i = 0; i < model_matrices.model_matrix_size(); ++i) {
    ...
    for (int j = 0; j < kNumMatrixEntries; j++) {
      ...
      new_matrix[row + col * 4] = std::isnan(model_matrix.matrix_entries(j))
                                      ? 0
                                      : model_matrix.matrix_entries(j);
    }
  }
}
...
absl::Status GlAnimationOverlayCalculator::Process(CalculatorContext *cc) {
  return helper_.RunInGlContext([this, &cc]() -> absl::Status {
    ...
    if (has_model_matrix_stream_ &&
      ...
      LOG(WARNING) << "Frame skipped: Don't have input model matrix";
      return absl::OkStatus();
    }
    ...
    // Load depth texture and ancors if they exist
    if (cc->Inputs().HasTag("DEPTH_TEXTURE") &&
        cc->Inputs().HasTag("ANCHORS")) {
      const auto &input_texture =
          cc->Inputs().Tag("DEPTH_TEXTURE").Get<AssetTextureFormat>();

      // test
      // cv::Mat depth_image_mat = formats::MatView(&input_texture);
      ...
      // depth_texture_ = helper_.CreateSourceTexture(input_texture);
      cv::Mat mat = formats::MatView(&input_texture);
      float *matData = (float *)mat.data;

      const std::vector<Anchor> anchors =
          cc->Inputs().Tag("ANCHORS").Get<std::vector<Anchor>>();
      anchors_depth_.clear();
      for (Anchor anchor : anchors) {
        anchors_depth_.push_back(anchor.z / 170.0f);
      }

      MP_RETURN_IF_ERROR(GlBind(current_frame, texture_, matData));
      if (has_model_matrix_stream_) {
        int i = 0;
        // Draw objects using our latest model matrix stream packet.
    ...
    // GLCHECK(glBindTexture(depth_texture_.target(), 0));
...
absl::Status GlAnimationOverlayCalculator::GlSetup() {
  ...
  const GLchar *vert_src = R"(
    ...
    void main() {
      ...
      vec2 fragCoords = (gl_Position.xy / gl_Position.w);
      screenCoords = fragCoords * 0.5 + 0.5; // Convert from [-1, 1] to [0, 1] to sample UV coordinates
    }
  )";
  ...
  const GLchar *frag_src = R"(
    ...
    // Depth processing  
    // передать DepthMat как текстуру и получить depth для текущего пикселя используя texture2D
    uniform sampler2D u_Depth; 
    uniform float u_AnchorDepth;
    ...
    // depth map.
    float GetVisibility(in vec2 depth_uv, in float depth_center_real) {
      float depth_relative = float(texture2D(u_Depth, depth_uv));
      float depth_center_relative = float(texture2D(u_Depth, vec2(0.5, 0.5)));
      
      float depth_real = depth_relative * depth_center_real / depth_center_relative;
      
      float visibility = clamp((1.34 - abs(depth_center_real - depth_real) * 5.0), 0.0, 1.0);

      if(abs(depth_center_real - depth_real) <= 0.07) {
        return 1.0;
      } else {
        return visibility;
      }
    ...
    void main() {
      ...
      gl_FragColor.a *= GetVisibility(screenCoords, u_AnchorDepth);
    }
  )";
  ...
  depth_texture_uniform_ = GLCHECK(glGetUniformLocation(program_, "u_Depth"));
  anchors_depth_uniform_ =
      GLCHECK(glGetUniformLocation(program_, "u_AnchorDepth"));
  return absl::OkStatus();
}

absl::Status
GlAnimationOverlayCalculator::GlBind(const TriangleMesh &triangle_mesh,
                                     const GlTexture &texture) {
...
GlAnimationOverlayCalculator::GlBind(const TriangleMesh &triangle_mesh,
  ...
  // gen depth texture
  GLCHECK(glGenTextures(1, &depth_texture_n_));
  GLCHECK(glBindTexture(GL_TEXTURE_2D, depth_texture_n_));
  ...
  GLCHECK(glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, 256, 256, 0, GL_RED, GL_FLOAT,
                       depth_texture));
  ...
  GLCHECK(glUniform1f(depth_texture_uniform_, 2));
...
absl::Status
GlAnimationOverlayCalculator::GlRender(const TriangleMesh &triangle_mesh,
                                       const float *model_matrix) {
...
GlAnimationOverlayCalculator::GlRender(const TriangleMesh &triangle_mesh,
  ...
  GLCHECK(glUniformMatrix4fv(model_matrix_uniform_, 1, GL_FALSE, model_matrix));
  GLCHECK(glUniform1fv(anchors_depth_uniform_, 1, &anchor_depth));
  GLCHECK(glDrawElements(GL_TRIANGLES, triangle_mesh.index_count,
                         GL_UNSIGNED_SHORT,
                         triangle_mesh.triangle_indices.get()));
  return absl::OkStatus();
